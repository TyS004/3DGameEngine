...
public:
	GameEngine()
	{
		...
		std::memset(m_keys, 0, 256 * sizeof(sKeyStatr44ex));
		m_mousePosX = 0;
		m_mousePosY = 0;
...
protected: // Audio Engine =====================================================================
		...
		unsigned int nBlocks = 8, unsigned int nBlockSamples = 512)
	{
		...
		m_AudioThread = std::thread(&GameEngine::AudioThread, this);
	...
	{
		((GameEngine*)dwInstance)->waveOutProc(hWaveOut, uMsg, dwParam1, dwParam2);
	}
